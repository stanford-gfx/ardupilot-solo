...
Vector3f AP_Gimbal::getGimbalRateDemVecYaw(const Quaternion &quatEst)
{
    ...
    float yaw_rate_ff = 0;
    if (_ahrs.get_gyro().z > _gimbalParams.get_K_rate()*yawErrorLimit) {
        yaw_rate_ff = _ahrs.get_gyro().z-_gimbalParams.get_K_rate()*yawErrorLimit;
    } else if (_ahrs.get_gyro().z < -_gimbalParams.get_K_rate()*yawErrorLimit) {
        yaw_rate_ff = _ahrs.get_gyro().z+_gimbalParams.get_K_rate()*yawErrorLimit;
    }
    ...
    gimbalRateDemVecYaw.z = yaw_rate_ff_filt - _gimbalParams.get_K_rate() * filtered_joint_angles.z;
